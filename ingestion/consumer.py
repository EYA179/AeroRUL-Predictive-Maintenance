...
BATCH_SIZE = 1000  # how many messages to accumulate per (dataset, part)
...
def flush_buffer(dataset, part):
    ...
    # Acknowledge only the messages that were written in this batch.
    for delivery_tag in tag_buffer:
    ...
    print(f"Flushed {len(df)} rows to table '{table_name}'.")

def flush_all_buffers():
    """
    Flushes all buffers for all (dataset, part) keys, useful on shutdown.
    """
    for ds, part in list(buffers.keys()):
        flush_buffer(ds, part)

def get_buffered_message_count():
    """Returns the total number of buffered, unacknowledged messages."""
    return sum(len(buffer["rows"]) for buffer in buffers.values())
...
def on_message(ch, method, properties, body):
    ...
    # Flush when one table buffer is full, or when total buffered messages
    # reaches the prefetch limit across multiple table buffers.
    if len(buffers[(dataset, part)]["rows"]) >= BATCH_SIZE:
        flush_buffer(dataset, part)
    elif get_buffered_message_count() >= BATCH_SIZE:
        flush_all_buffers()
