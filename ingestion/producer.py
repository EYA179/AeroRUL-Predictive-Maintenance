...
def produce_all_data(base_folder):
    ...
    for ds in FD_DATASETS:
        ...
        missing_files = [path for path in [train_file, test_file, rul_file] if not os.path.isfile(path)]
        if missing_files:
            raise FileNotFoundError(f"Missing files for {ds}: {', '.join(missing_files)}")
...
if __name__ == "__main__":
    ...
    base_folder = project_root / "data" / "raw"
